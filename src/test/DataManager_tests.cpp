...
int main() {
    ...
    key = DataManager::finish();
    DataManager::init();
    DataManager::load(key);

    util::map<std::string, Blob> newTestMap = util::map<std::string, Blob>::load(testMap.getId());
    unsigned cnt = 0;
...
*/
    ...
    util::map<Blob, Blob> newEmptyMap = util::map<Blob, Blob>::load(emptyMap.getId());
