...
int main() {
 ...
 //       if (i < 10) std::cout << i << ": " << routeId[i] << std::endl;
		...
		st.add(stationName[i / 2], today, RouteInterval(routeId[i], routeName[i], 0, 1));
	}
	cout << "add test ok" << endl;

	for(int i = 50; i < 100; ++i) {
        //    std::cout << stationName[i / 2] << " " << routeName[i] << std::endl;
		st.del(stationName[i / 2], today, RouteInterval(routeId[i], routeName[i], 0, 1));
	}
	...
	try {
		st.del(stationName[0], today, RouteInterval(Id("Route"), getSalt(4), 0, 1));
    ...
    for (int i = 0; i < 25; ++ i) {
        // std::cout << i << std::endl;
        const auto routes = st.query(stationName[i], today);
...
//        std::cout << routes[0] << " " << routes[1] << std::endl;
    ...
    const auto route = st.query(stationName[0], today, routeName[0]);
    ...
    try {
		new_st.del(stationName[0], today, RouteInterval(Id("Route"), getSalt(4), 0, 1));
        std::cout << "throw test for del failed!" << std::endl;
	} catch (const Station::delete_routeId_failed& e) {
		std::cout << "throw test for del ok!" << std::endl;
	}
    ...
    for (int i = 0; i < 25; ++ i) {
        // std::cout << i << std::endl;
        const auto routes = new_st.query(stationName[i], today);
        assert(routes.size() == 2);
//        std::cout << routes[0] << " " << routes[1] << std::endl;
        assert((routes[0].routeId == routeId[i * 2] && routes[1].routeId == routeId[i * 2 + 1]) || (routes[1].routeId == routeId[i * 2] && routes[0].routeId == routeId[i * 2 + 1]));
    }
    const auto route2 = new_st.query(stationName[0], today, routeName[0]);
