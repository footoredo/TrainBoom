#ifndef TRAINBOOM_UTIL_SET_HPP
#define TRAINBOOM_UTIL_SET_HPP
...
public:
	...
	public:
		...
		iterator(const stupid_ptr<Node>& p, const set *who): p(p), who(who) {
		}
	...
	class const_iterator {
		...
		public:
			...
			const_iterator(const stupid_ptr<Node>& p, const set *who): p(p), who(who) {}
	...
	pair<iterator, bool> insert(const value_type &value) {
		...
		if (tmp.second) {
			++ _size;
			if (!min_p || Compare()(value, *(min_p->value)))
				min_p = tmp.first;
			if (!max_p || Compare()(*(max_p->value), value))
				max_p = tmp.first;
...
private:
	...
	stupid_ptr<Node> find_next(const stupid_ptr<Node>& u) const {
		...
		stupid_ptr<Node> cur(root), ans(nullptr);
		while (cur) {
			if (Compare()(key, *(cur->value))) {
				if (!ans || Compare()(*(cur->value), *(ans->value))) {
					ans = cur;
				}
				cur = cur->child[0];
			}
	...
	stupid_ptr<Node> _find(const Key& key) const {
		...
		while (u) {
			if (Compare()(key, *(u->value))) {
				u = u->child[0];
			}
			else if (Compare()(*(u->value), key)) {
				u = u->child[1];
			}
			else {
				return u;
	...
	void _delete(stupid_ptr<Node> u) {
		...
		if (u->color == BLACK) {
			if (child && child->color == RED) {
				...
				u = child;
				while (true) {
					// std::cout << "deleting " << *(u->value) << std::endl;
					if (!p)
						break;
					else {
